...
// Helpdesk profile: remote control, terminal, wake, notes, remote view.
const MESH_RIGHTS_HELPDESK = 0x000001B8;
...
function removeDbRecord(db, id) {
  return new Promise((resolve, reject) => {
    if (!db || typeof db.Remove !== "function" || !id) {
      reject(new Error("MeshCentral DB remove API unavailable."));
      return;
    }

    try {
      if (db.Remove.length >= 2) {
...
function extractMeshIdCandidatesFromImageId(imageId) {
  ...
  const regex = /mesh\/[^/\s]+\/[^,\s"']+/g;
  let match;
  while ((match = regex.exec(text)) !== null) {
    if (match[0] && candidates.indexOf(match[0]) === -1) {
...
function resolveImageconfigMesh(meshServer, domainId, imageId) {
  const candidates = extractMeshIdCandidatesFromImageId(imageId);
  for (const candidate of candidates) {
    const mesh = resolveTargetMesh(meshServer, domainId, candidate);
    if (!mesh || typeof mesh !== "object") continue;
    if (String(mesh.domain || "") !== String(domainId || "")) continue;
    return mesh;
  }
  return null;
}

async function listNodesInMesh(db, meshServer, domainId, meshId) {
  const nodesById = new Map();
  const cachedNodes = (meshServer && meshServer.nodes && typeof meshServer.nodes === "object") ? meshServer.nodes : {};
  for (const key in cachedNodes) {
    if (!Object.prototype.hasOwnProperty.call(cachedNodes, key)) continue;
    const node = cachedNodes[key];
    if (!node || typeof node !== "object" || typeof node._id !== "string") continue;
    nodesById.set(node._id, node);
  }

  const dbNodes = await getAllTypeRecords(db, "node");
  ...
  const result = [];
  const wantedDomain = String(domainId || "");
  for (const node of nodesById.values()) {
    if (String(node.meshid || "") !== meshId) continue;

    const nodeDomain = String(node.domain || "");
    if (nodeDomain && nodeDomain !== wantedDomain) continue;
    if (!nodeDomain && typeof node._id === "string" && wantedDomain && !node._id.startsWith(`node/${wantedDomain}/`)) continue;
...
async function unlinkMeshFromEntityLinks(db, meshServer, meshId, domainId) {
  const users = []
  ...
  for (const user of users) {
    if (!user || typeof user !== "object" || typeof user._id !== "string" || !user._id) continue;
    userMap.set(user._id, user);
  }

  for (const user of userMap.values()) {
    if (domainId && !String(user._id).startsWith(`user/${domainId}/`)) continue;
    const links = (user.links && typeof user.links === "object") ? user.links : null;
    if (!links || !Object.prototype.hasOwnProperty.call(links, meshId)) continue;
  ...
  const groups = []
    .concat(Object.values((meshServer && meshServer.userGroups && typeof meshServer.userGroups === "object") ? meshServer.userGroups : {}))
    .concat(await getAllTypeRecords(db, "ugrp"));
  const groupMap = new Map();
  for (const group of groups) {
    if (!group || typeof group !== "object" || typeof group._id !== "string" || !group._id) continue;
    groupMap.set(group._id, group);
  }

  for (const group of groupMap.values()) {
    if (domainId && !String(group._id).startsWith(`ugrp/${domainId}/`)) continue;
    const links = (group.links && typeof group.links === "object") ? group.links : null;
    if (!links || !Object.prototype.hasOwnProperty.call(links, meshId)) continue;

    delete links[meshId];
    db.Set(group);
    syncCachedEntity(meshServer, "userGroups", group._id, group);
  }
}

async function deleteMeshRecord(db, meshServer, meshId, domainId) {
  await unlinkMeshFromEntityLinks(db, meshServer, meshId, domainId);
  await removeDbRecord(db, meshId);

  if (meshServer && meshServer.meshes && typeof meshServer.meshes === "object") {
    delete meshServer.meshes[meshId];
  }

  if (meshServer && meshServer.webserver && meshServer.webserver.meshes && typeof meshServer.webserver.meshes === "object") {
    delete meshServer.webserver.meshes[meshId];
  }
}
...
function buildDefaultMeshPrincipals(adminUser, domainId) {
  const principals = [];
  if (adminUser && typeof adminUser._id === "string" && adminUser._id.length > 0) {
    principals.push({
      id: adminUser._id,
      name: (typeof adminUser.name === "string" && adminUser.name.trim().length > 0) ? adminUser.name.trim() : "admin",
...
async function getDomainAdminUser(db, meshServer, domainId) {
  const adminId = `user/${String(domainId || "").trim()}/admin`;
  if (meshServer && meshServer.users && typeof meshServer.users === "object") {
    const cached = meshServer.users[adminId];
    if (cached && typeof cached === "object") return cached;
  }

  const users = await getAllTypeRecords(db, "user");
  for (const user of users) {
    if (!user || typeof user !== "object") continue;
    if (String(user._id || "") === adminId) return user;
  }
...
function createMeshcentralService(meshServer, debug) {
  ...
  async function syncCreatedImageconfigGroups(domainId, createdImageconfigs, user) {
    ...
    const adminUser = await getDomainAdminUser(db, meshServer, domainId);
    if (!adminUser || typeof adminUser._id !== "string" || adminUser._id.length === 0) {
      outcome.blocked.push({
        domainId: String(domainId || "default"),
        adminUserId: `user/${String(domainId || "").trim()}/admin`,
        reason: `Required domain admin user user/${String(domainId || "").trim()}/admin was not found.`
      });
      return outcome;
    }

    const principals = buildDefaultMeshPrincipals(adminUser, domainId);
    const meshPrincipalLinks = buildMeshPrincipalLinks(principals);
    const creatorId = String(adminUser._id || "");
    const creatorName = (typeof adminUser.name === "string" && adminUser.name.trim().length > 0) ? adminUser.name.trim() : "admin";
    if (!os2Group) {
          ...
          for (const principal of principals) {
            await ensurePrincipalMeshLink(db, meshServer, {
              ...
              type: principal.id.startsWith("ugrp/") ? "group" : "user",
                entity: principal.id === creatorId ? adminUser : null
            }, existing._id);
          }
  ...
  async function syncDeletedImageconfigGroups(domainId, deletedImageconfigs) {
    const outcome = {
      checked: 0,
      deleted: 0,
      skipped: 0,
    ...
    const items = Array.isArray(deletedImageconfigs) ? deletedImageconfigs : [];
    if (items.length === 0) return outcome;

    const db = meshServer && meshServer.db;
    if (!db) {
      outcome.warnings.push("MeshCentral DB API unavailable; skipped imageconfig group deletion sync.");
      return outcome;
    }

    for (const item of items) {
      const imageconfigPath = String(item && item.path ? item.path : "");
      const imageId = String(item && item.imageId ? item.imageId : "").trim();
      const imageconfigName = String(item && item.groupName ? item.groupName : "").trim() || imageconfigPath;
      ...
      const mesh = resolveImageconfigMesh(meshServer, domainId, imageId);
      if (!mesh || typeof mesh !== "object" || !mesh._id) {
        outcome.skipped += 1;
        continue;
      }
      ...
      const nodes = await listNodesInMesh(db, meshServer, domainId, meshId);
      if (nodes.length > 0) {
        outcome.blocked.push({
          path: imageconfigPath,
          imageconfigName,
          meshId,
          meshName: String(mesh.name || meshId),
          deviceCount: nodes.length,
          reason: "Device group is not empty."
        });
        continue;
      }

      try {
        await deleteMeshRecord(db, meshServer, meshId, domainId);
        outcome.deleted += 1;
      } catch (error) {
        const message = String((error && error.message) ? error.message : error || "Unknown delete error");
        outcome.blocked.push({
          path: imageconfigPath,
          imageconfigName,
          meshId,
          meshName: String(mesh.name || meshId),
          deviceCount: 0,
          reason: `Failed to delete device group: ${message}`
        });
      }
    }

...
function createConfigService(options) {
  ...
  async function prepareSave(req, user, body) {
    ...
    const [existingPolicyPaths, existingImageconfigFiles] = await Promise.all([
    ...
    for (const existingFile of existingImageconfigFiles) {
      const existingPath = normalizePath(existingFile && existingFile.path);
      ...
      if (!submittedImageconfigPaths.has(existingPath)) {
        ...
        deletedImageconfigs.push({
          path: existingPath,
          groupName: extractImageconfigGroupName(existingPath, existingFile && existingFile.data),
          imageId: extractImageId(existingFile && existingFile.data)
        });
      }
    }
  ...
  function applyImageIdMappingsToFileChanges(fileChanges, mappings) {
    const mappingList = Array.isArray(mappings) ? mappings : [];
    const changes = Array.isArray(fileChanges) ? fileChanges : [];
    const byPath = new Map();

    for (const mapping of mappingList) {
      if (!mapping || typeof mapping.path !== "string") continue;
      const pathKey = normalizePath(mapping.path);
      if (!pathKey) continue;
      const meshId = String(mapping.meshId || "").trim();
      if (!meshId) continue;
      byPath.set(pathKey, meshId);
    }
    ...
    for (const change of changes) {
      if (!change || change.delete === true || typeof change.path !== "string" || typeof change.contentUtf8 !== "string") continue;
      const pathKey = normalizePath(change.path);
      const meshId = byPath.get(pathKey);
      if (!meshId) continue;

      try {
        const parsed = parseConfigFile(pathKey, change.contentUtf8);
        const content = (parsed && typeof parsed === "object") ? parsed : {};
        content.image_id = meshId;
        change.contentUtf8 = stringifyConfigFile(pathKey, content);
        updatedPaths.push(pathKey);
      } catch (error) {
        warnings.push(`Failed to set image_id for ${pathKey}: ${error.message || error}`);
      }
    }
  ...
  return {
    ...
    getBootstrapPayload,
    prepareSave,
    applyImageIdMappingsToFileChanges
  };
}
